...
for j in range(0,leng):
    IGAP_OD_out = Xod * (((OBCAL[j, 2] - WQD[2*j + 1, 6]) / (WQD[2*j, 6]))) * (L[j] * Rod / vel[j])
    IGAP_DBO_out = Xdbo * ((OBCAL[j, 3] - WQD[2 * j + 1, 7]) / (WQD[2 * j, 7] )) * (L[j] * Rdbo / vel[j])
    IGAP_DQO_out = Xdqo * ((OBCAL[j, 4] - WQD[2 * j + 1, 8]) / (WQD[2 * j, 8])) * (L[j] * Rdqo / vel[j])
    IGAP_NH3_out = Xnh3 * ((OBCAL[j, 7] - WQD[2 * j + 1, 11]) / (WQD[2 * j, 11])) * (L[j] * Rnh3 / vel[j])
    IGAP_NO2_out = Xno2 * ((OBCAL[j, 6] - WQD[2 * j + 1, 10]) / (WQD[2 * j, 10])) * (L[j] * Rno2 / vel[j])
    IGAP_NO3_out = Xno3 * ((OBCAL[j, 5] - WQD[2 * j + 1, 9]) / (WQD[2 * j, 9])) * (L[j] * Rno3 / vel[j])
    IGAP_Por_out = Xporg * ((OBCAL[j, 9] - WQD[2 * j + 1, 13]) / (WQD[2 * j, 13])) * (L[j] * Rporg / vel[j])
    IGAP_pH_out = Xph * ((OBCAL[j, 0] - WQD[2 * j + 1, 4]) / (WQD[2 * j, 4])) * (L[j] * RpH / vel[j])
    IGAP_Con_out = Xcon * ((OBCAL[j, 1] - WQD[2 * j + 1, 5]) / (WQD[2 * j, 5])) * (L[j] * Rcon / vel[j])
    IGAP_SST_out = Xsst * ((OBCAL[j, 8] - WQD[2 * j + 1, 12]) / (WQD[2 * j, 12])) * (L[j] * Rsst / vel[j])
...
Salida.to_excel(r'C:\Users\willy\OneDrive\Escritorio\Curso_Python\IGAP\Salida_IGAP.xlsx', index=False)
