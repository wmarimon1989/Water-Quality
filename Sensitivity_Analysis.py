...
for j in range(leng2):
      Si = sobol.analyze(problem, Y[:,j], calc_second_order=False, num_resamples=100, conf_level=0.95, print_to_console=False)
      S1[j,:] = Si['S1']
...
#print(ST_conf)
IGAP = cal_IGAP(SAV)
SA = np.concatenate((S1, S1_conf, ST,ST_conf), axis=1)
...
Salida_SA = pd.DataFrame(SA, columns=['S1_VAR1', 'S1_VAR2', 'S1_conf_VAR1', 'S1_conf_VAR2', 'ST_VAR1', 'ST_VAR2', 'ST_conf_VAR1', 'ST_conf_VAR2'])
