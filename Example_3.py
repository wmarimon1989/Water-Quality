...
def read_config_file(archivo_entrada):
    """
    ...
    WD = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='WD')
    wd = WD.to_numpy()

    # Reading water bed slope
    SL = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SL')
    sl = SL.to_numpy()

    # Reading water velocities sheet
    WV = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='WV')
    wv = WV.to_numpy()

    # Reading boundary conditions sheet
    BC = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='BC')
    bc = BC.to_numpy()

    # Reading initial conditions sheet
    IC = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='IC')
    ic = IC.to_numpy()

    # Reading sinks and sources sheet
    sod = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SOD')
    SOD = sod.to_numpy()
    SOD = SOD[:, 1:]

    # Reading sinks and sources sheet
    sdbo = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SDBO')
    SDBO = sdbo.to_numpy()
    SDBO = SDBO[:, 1:]

    # Reading sinks and sources sheet
    snh3 = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SNH4')
    SNH3 = snh3.to_numpy()
    SNH3 = SNH3[:, 1:]

    # Reading sinks and sources sheet
    sno2 = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SNO2')
    SNO2 = sno2.to_numpy()
    SNO2 = SNO2[:, 1:]

    # Reading sinks and sources sheet
    sdqo = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SDQO')
    SDQO = sdqo.to_numpy()
    SDQO = SDQO[:, 1:]

    # Reading sinks and sources sheet
    stds = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='STDS')
    STDS = stds.to_numpy()
    STDS = STDS[:, 1:]

    # Reading sinks and sources sheet
    sgya = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SGyA')
    SGyA = sgya.to_numpy()
    SGyA = SGyA[:, 1:]

    # Reading sinks and sources sheet
    sno3 = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SNO3')
    SNO3 = sno3.to_numpy()
    SNO3 = SNO3[:, 1:]

    # Reading sinks and sources sheet
    sec = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SEC')
    SEC = sec.to_numpy()
    SEC = SEC[:, 1:]

    # Reading sinks and sources sheet
    stc = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='STC')
    STC = stc.to_numpy()
    STC = STC[:, 1:]

    # Reading sinks and sources sheet
    sporg = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SPorg')
    SPorg = sporg.to_numpy()
    SPorg = SPorg[:, 1:]

    # Reading sinks and sources sheet
    spdis = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SPdis')
    SPdis = spdis.to_numpy()
    SPdis = SPdis[:, 1:]

    # Reading sinks and sources sheet
    stss = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='STSS')
    STSS = stss.to_numpy()
    STSS = STSS[:, 1:]

    # Reading sinks and sources sheet
    sss = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SSS')
    SSS = sss.to_numpy()
    SSS = SSS[:, 1:]

    # Reading sinks and sources sheet
    st = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='ST')
    ST = st.to_numpy()
    ST = ST[:, 1:]

    # Reading sinks and sources sheet
    sph = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SpH')
    SpH = sph.to_numpy()
    SpH = SpH[:, 1:]

    # Reading sinks and sources sheet
    salk = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='SALK')
    SALK = salk.to_numpy()
    SALK = SALK[:, 1:]

    # Leer caudales primera fila no puede contener ceros
    # La verificación se hace cuando se carga el archivo
    caudales = pd.ExcelFile(io=archivo_entrada).parse(sheet_name='Caudales')
    Caudales = caudales.to_numpy()
    Caudales = Caudales[:, 1:]
...
def calidad_explicito(D, dx, ci_T, ci_OD, ci_DBO, ci_NH3, ci_NO2, ci_NO3, ci_DQO, ci_TDS, ci_EC, ci_TC, ci_GyA, ci_Porg, ci_Pdis, ci_TSS,
    ...
    for i in range(int(dtini / dt)):
        ...
        cout_T[-1] = cout_T[-2]*(1 -(Aair*Tair*sbc))
        cout_OD[-1] = cout_OD[-2]*(1-fdw*(kdbo+kDQO+ko2+knh3))
        cout_DBO[-1] = cout_DBO[-2]*(1-fdw*(kdbo+kDQO+ko2+knh3))
        cout_NH3[-1] = cout_NH3[-2]*(1-fdw*(kdbo+kDQO+ko2+knh3))
        cout_OD[-1] = cout_OD[-2]*(1-fdw*(kdbo+kDQO+ko2+knh3))
        cout_NO2[-1] = cout_NO2[-2]*(1-fdw*(kdbo+kDQO+ko2+knh3))
        cout_NO3[-1] = cout_NO3[-2]*(1-fdw*(kdbo+kDQO+ko2+knh3))
        cout_DQO[-1] = cout_DQO[-2]*(1-fdw*(kdbo+kDQO+ko2+knh3))
        cout_TDS[-1] = cout_TDS[-2]*(1-fdw*(kTDS))
        cout_EC[-1] = cout_EC[-2]*(1-fdw*(kEC))
        cout_TC[-1] = cout_TC[-2]*(1-fdw*(kTC))
        cout_GyA[-1] = cout_GyA[-2]*(1-fdw*(kH+kf))
        cout_Porg[-1] = cout_Porg[-2]*(1-fdw*(kPorg+kPsed))
        cout_Pdis[-1] = cout_Pdis[-2]*(1-fdw*(kPorg+kPsed))
        cout_SS[-1] = cout_SS[-2]*(1-fdw*(ks))
        cout_pH[-1] = cout_pH[-2]*(1-fdw*(kH))
        cout_ALK[-1] = cout_ALK[-2]*(1-fdw*(kH))
...
if __name__ == '__main__':
    archivo_entrada = r'C:\Users\willy\OneDrive\Escritorio\Curso_Python\Calidad_CAR\Prueba_CAR.xls'
    tiempo = 86400
